...
// CHECK: cir.func {{.*}} @shouldConstInitStructs
  ...
  // CHECK: %[[#V1:]] = cir.cast bitcast %[[#V0]] : !cir.ptr<!rec_Foo> -> !cir.ptr<!rec_anon_struct{{[0-9]*}}>
  // CHECK: %[[#V2:]] = cir.get_global @__const.shouldConstInitStructs.f : !cir.ptr<!rec_anon_struct{{[0-9]*}}>
  // CHECK: cir.copy %[[#V2]] to %[[#V1]] : !cir.ptr<!rec_anon_struct{{[0-9]*}}>
}
...
// CHECK-DAG: cir.func {{.*}} @shouldCopyStructAsCallArg
  ...
  // CHECK-DAG: %[[#LV0:]] = cir.load %{{.+}} : !cir.ptr<!u64i>, !u64i
  // CHECK-DAG: %[[#LV1:]] = cir.load %{{.+}} : !cir.ptr<!u64i>, !u64i
  // CHECK-DAG: cir.call @shouldCopyStructAsCallArg(%[[#LV0]], %[[#LV1]]) : (!u64i, !u64i) -> ()
}
