...
// CIR: [[INTRES0:%.*]] = cir.call_llvm_intrinsic "aarch64.ldxr" {{%[0-9]+}} : (!cir.ptr<!s8i>) -> !s64i
// CIR: [[CAST0:%.*]] = cir.cast integral [[INTRES0]] : !s64i -> !s8i
// CIR: [[CAST_I32:%.*]] = cir.cast integral [[CAST0]] : !s8i -> !s32i

  sum += __builtin_arm_ldrex((short *)addr);
// CIR: [[INTRES1:%.*]] = cir.call_llvm_intrinsic "aarch64.ldxr" {{%[0-9]+}} : (!cir.ptr<!s16i>) -> !s64i
// CIR: [[CAST1:%.*]] = cir.cast integral [[INTRES1]] : !s64i -> !s16i
// CIR: [[CAST_I16:%.*]] = cir.cast integral [[CAST1]] : !s16i -> !s32i

  sum += __builtin_arm_ldrex((int *)addr);
// CIR: [[INTRES2:%.*]] = cir.call_llvm_intrinsic "aarch64.ldxr" {{%[0-9]+}} : (!cir.ptr<!s32i>) -> !s64i
// CIR: [[CAST2:%.*]] = cir.cast integral [[INTRES2]] : !s64i -> !s32i

  sum += __builtin_arm_ldrex((long long *)addr);
// CIR: [[INTRES3:%.*]] = cir.call_llvm_intrinsic "aarch64.ldxr" {{%[0-9]+}} : (!cir.ptr<!s64i>) -> !s64i

  sum += __builtin_arm_ldrex(addr64);
// CIR: [[INTRES4:%.*]] = cir.call_llvm_intrinsic "aarch64.ldxr" {{%[0-9]+}} : (!cir.ptr<!s64i>) -> !s64i


  sum += *__builtin_arm_ldrex((int **)addr);
// CIR: [[INTRES5:%.*]] = cir.call_llvm_intrinsic "aarch64.ldxr"  {{%[0-9]+}} : (!cir.ptr<!cir.ptr<!s32i>>) -> !s64i

  sum += __builtin_arm_ldrex((struct twoFldT **)addr)->a;
// CIR: [[INTRES6:%.*]] = cir.call_llvm_intrinsic "aarch64.ldxr"  {{%[0-9]+}} : (!cir.ptr<!cir.ptr<!rec_twoFldT>>) -> !s64i
...
// CIR: [[INTRES0:%.*]] = cir.call_llvm_intrinsic "aarch64.ldaxr" {{%[0-9]+}} : (!cir.ptr<!s8i>) -> !s64i
// CIR: [[CAST0:%.*]] = cir.cast integral [[INTRES0]] : !s64i -> !s8i
// CIR: [[CAST_I32:%.*]] = cir.cast integral [[CAST0]] : !s8i -> !s32i

  sum += __builtin_arm_ldaex((short *)addr);
// CIR: [[INTRES1:%.*]] = cir.call_llvm_intrinsic "aarch64.ldaxr" {{%[0-9]+}} : (!cir.ptr<!s16i>) -> !s64i
// CIR: [[CAST1:%.*]] = cir.cast integral [[INTRES1]] : !s64i -> !s16i
// CIR: [[CAST_I16:%.*]] = cir.cast integral [[CAST1]] : !s16i -> !s32i

  sum += __builtin_arm_ldaex((int *)addr);
// CIR: [[INTRES2:%.*]] = cir.call_llvm_intrinsic "aarch64.ldaxr" {{%[0-9]+}} : (!cir.ptr<!s32i>) -> !s64i
// CIR: [[CAST2:%.*]] = cir.cast integral [[INTRES2]] : !s64i -> !s32i

  sum += __builtin_arm_ldaex((long long *)addr);
// CIR: [[INTRES3:%.*]] = cir.call_llvm_intrinsic "aarch64.ldaxr" {{%[0-9]+}} : (!cir.ptr<!s64i>) -> !s64i

  sum += __builtin_arm_ldaex(addr64);
// CIR: [[INTRES4:%.*]] = cir.call_llvm_intrinsic "aarch64.ldaxr" {{%[0-9]+}} : (!cir.ptr<!s64i>) -> !s64i


  sum += *__builtin_arm_ldaex((int **)addr);
// CIR: [[INTRES5:%.*]] = cir.call_llvm_intrinsic "aarch64.ldaxr"  {{%[0-9]+}} : (!cir.ptr<!cir.ptr<!s32i>>) -> !s64i

  sum += __builtin_arm_ldaex((struct twoFldT **)addr)->a;
// CIR: [[INTRES6:%.*]] = cir.call_llvm_intrinsic "aarch64.ldaxr"  {{%[0-9]+}} : (!cir.ptr<!cir.ptr<!rec_twoFldT>>) -> !s64i
// CIR: [[CAST3:%.*]] = cir.cast int_to_ptr [[INTRES6]] : !s64i -> !cir.ptr<!rec_twoFldT>
// CIR: [[MEMBER_A:%.*]] = cir.get_member [[CAST3]][0] {name = "a"} : !cir.ptr<!rec_twoFldT> -> !cir.ptr<!s8i>
