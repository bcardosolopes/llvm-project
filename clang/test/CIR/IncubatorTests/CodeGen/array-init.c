...
// CIR-SAME: }>
// LLVM: @outers = global
// LLVM-SAME: { i32, [4 x i8], [1 x { i64, i32, [4 x i8] }] } { i32 1, [4 x i8] zeroinitializer, [1 x { i64, i32, [4 x i8] }] [{ i64, i32, [4 x i8] } { i64 0, i32 1, [4 x i8] zeroinitializer }] },
// LLVM-SAME: { i32, [4 x i8], [1 x %struct.Inner] } { i32 1, [4 x i8] zeroinitializer, [1 x %struct.Inner] zeroinitializer }
...
// CIR-NEXT: [[SE_EL:%.*]] = cir.ptr_stride [[FI_EL]], [[ONE]] : (!cir.ptr<!rec_T>, !s64i) -> !cir.ptr<!rec_T>
...
// CIR-NEXT: [[B1_ZERO:%.*]] = cir.const #cir.int<0> : !s64i
// CIR-NEXT: cir.store{{.*}} [[B1_ZERO]], [[B_STORAGE1]] : !s64i, !cir.ptr<!s64i>
// CIR-NEXT: cir.return
...
// CIR-NEXT: [[ELEM0:%.*]] = cir.cast array_to_ptrdecay [[ARR]] : !cir.ptr<!cir.array<!s32i x 3>> -> !cir.ptr<!s32i>
// CIR-NEXT: [[LOAD_A:%.*]] = cir.load{{.*}} [[A]] : !cir.ptr<!s32i>, !s32i
// CIR-NEXT: cir.store{{.*}} [[LOAD_A]], [[ELEM0]] : !s32i, !cir.ptr<!s32i>
// CIR-NEXT: [[ONE:%.*]] = cir.const #cir.int<1> : !s64i
// CIR-NEXT: [[ELEM1:%.*]] = cir.ptr_stride [[ELEM0]], [[ONE]] : (!cir.ptr<!s32i>, !s64i) -> !cir.ptr<!s32i>
...
// CIR-NEXT: [[ELEM2:%.*]] = cir.ptr_stride [[ELEM0]], [[TWO]] : (!cir.ptr<!s32i>, !s64i) -> !cir.ptr<!s32i>
...
// CIR:    [[BEGIN:%.*]] = cir.cast array_to_ptrdecay [[ARR]] : !cir.ptr<!cir.array<!s32i x 3>> -> !cir.ptr<!s32i>
// CIR:    [[VAR:%.*]] = cir.load{{.*}} [[VAR_ALLOC]] : !cir.ptr<!s32i>, !s32i
// CIR:    cir.store{{.*}} [[VAR]], [[BEGIN]] : !s32i, !cir.ptr<!s32i>
// CIR:    [[ONE:%.*]] = cir.const #cir.int<1> : !s64i
// CIR:    [[ZERO_INIT_START:%.*]] = cir.ptr_stride [[BEGIN]], [[ONE]] : (!cir.ptr<!s32i>, !s64i) -> !cir.ptr<!s32i>
// CIR:    cir.store{{.*}} [[ZERO_INIT_START]], [[TEMP]] : !cir.ptr<!s32i>, !cir.ptr<!cir.ptr<!s32i>>
// CIR:    [[SIZE:%.*]] = cir.const #cir.int<3> : !s64i
// CIR:    [[END:%.*]] = cir.ptr_stride [[BEGIN]], [[SIZE]] : (!cir.ptr<!s32i>, !s64i) -> !cir.ptr<!s32i>
// CIR:    cir.do {
...
// CIR:    [[ELEM1:%.*]] = cir.ptr_stride [[BEGIN]], [[OFFSET1]] : (!cir.ptr<!s32i>, !s64i) -> !cir.ptr<!s32i>
// CIR:    [[TWO:%.*]] = cir.const #cir.int<2> : !s32i
// CIR:    cir.store{{.*}} [[TWO]], [[ELEM1]] : !s32i, !cir.ptr<!s32i>
// CIR:    [[OFFSET2:%.*]] = cir.const #cir.int<2> : !s64i
// CIR:    [[ELEM2:%.*]] = cir.ptr_stride [[BEGIN]], [[OFFSET2]] : (!cir.ptr<!s32i>, !s64i) -> !cir.ptr<!s32i>
// CIR:    [[THREE:%.*]] = cir.const #cir.int<3> : !s32i
// CIR:    cir.store{{.*}} [[THREE]], [[ELEM2]] : !s32i, !cir.ptr<!s32i>
// CIR:    [[OFFSET3:%.*]] = cir.const #cir.int<3> : !s64i
// CIR:    [[ELEM3:%.*]] = cir.ptr_stride [[BEGIN]], [[OFFSET3]] : (!cir.ptr<!s32i>, !s64i) -> !cir.ptr<!s32i>
// CIR:    [[VAR:%.*]] = cir.load{{.*}} [[VAR_ALLOC]] : !cir.ptr<!s32i>, !s32i
// CIR:    cir.store{{.*}} [[VAR]], [[ELEM3]] : !s32i, !cir.ptr<!s32i>
// CIR:    [[ONE_VAR:%.*]] = cir.const #cir.int<1> : !s64i
// CIR:    [[LAST:%.*]] = cir.ptr_stride [[ELEM3]], [[ONE_VAR]] : (!cir.ptr<!s32i>, !s64i) -> !cir.ptr<!s32i>
...
// LLVM:   [[LAST:%.*]] = getelementptr i32, ptr [[THREE]], i64 1
