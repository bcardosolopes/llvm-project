// RUN: %clang_cc1 %s -fclangir -triple=spirv64-unknown-unknown -cl-opt-disable -emit-cir -fno-clangir-call-conv-lowering -o %t.cir -ffake-address-space-map
// RUN: FileCheck -input-file=%t.cir -check-prefix=CIR %s
// RUN: %clang_cc1 %s -fclangir -triple=spirv64-unknown-unknown -cl-opt-disable -emit-llvm -fno-clangir-call-conv-lowering -o %t.ll -ffake-address-space-map
...
// CIR: cir.global{{.*}} constant {{.*}}lang_address_space(offload_constant) @{{.*}} = #cir.const_array<"f\00" : !cir.array<!s8i x 2>> : !cir.array<!s8i x 2>
