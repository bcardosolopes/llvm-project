...
// TODO: LLVM-DEVICE is disabled because AMDGPU requires addrspace(5) for alloca
// which CIR lowering doesn't support yet.
// DISABLED: %clang_cc1 -triple=amdgcn-amd-amdhsa -x hip -fclangir \
// DISABLED:            -fcuda-is-device -fhip-new-launch-api    \
// DISABLED:            -fvisibility=hidden                      \
// DISABLED:            -I%S/../Inputs/ -emit-llvm %s -o %t.ll
// DISABLED: FileCheck --check-prefix=LLVM-DEVICE --input-file=%t.ll %s
