...
double f1(int n, ...) {
  ...
  __builtin_va_start(valist, n);
  double res = __builtin_va_arg(valist, double);
...
// BEFORE:  cir.va_start [[VARLIST:%[a-z0-9]+]]
// BEFORE:  [[TMP0:%.*]] = cir.va_arg [[VARLIST]] : (!cir.ptr<!rec___va_list>) -> !cir.double
// BEFORE:  cir.store{{.*}} [[TMP0]], [[RESP]] : !cir.double, !cir.ptr<!cir.double>
// BEFORE:  cir.va_end [[VARLIST]] : !cir.ptr<!rec___va_list>
...
// AFTER:  cir.va_start [[VARLIST:%[a-z0-9]+]]
...
// AFTER:   cir.va_end [[VARLIST]] : !cir.ptr<!rec___va_list>
