...
__device__ void nvvm_sync(unsigned mask, int i, float f, int a, int b,
  ...
  // CIR: cir.call_llvm_intrinsic "nvvm.bar.warp.sync" {{.*}} : (!u32i)
  // LLVM: call void @llvm.nvvm.bar.warp.sync(i32
  // OGCHECK: call void @llvm.nvvm.bar.warp.sync(i32
  __nvvm_bar_warp_sync(mask);

  // CIR: cir.call_llvm_intrinsic "nvvm.barrier.cta.sync.all" {{.*}} : (!u32i)
  ...
  // CIR: cir.call_llvm_intrinsic "nvvm.barrier.cta.sync.count" {{.*}} : (!u32i, !u32i)
