...
Sema::createLambdaClosureType(SourceRange IntroducerRange, TypeSourceInfo *Info,
  ...
  // Do not skip over a RequiresExprBodyDecl: a lambda in a requires-expression
  // must keep it as its semantic context so that unqualified lookup in the
  // lambda body finds the enclosing template parameters (GH147650).
