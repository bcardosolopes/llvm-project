...
namespace mlir {
#define GEN_PASS_DEF_LIVEOBJECTDIAGNOSTIC
#include "clang/CIR/Dialect/Passes.h.inc"
} // namespace mlir

struct LiveObjectDiagnosticPass
    : public mlir::impl::LiveObjectDiagnosticBase<LiveObjectDiagnosticPass> {
...
// Find the innermost breakable (loop or switch) parent for a break op.
static mlir::Operation *getBreakTarget(cir::BreakOp breakOp) {
  mlir::Operation *parent = breakOp->getParentOp();
  while (parent) {
    if (mlir::isa<cir::LoopOpInterface, cir::SwitchOp>(parent))
      return parent;
...
void LiveObjectAnalysis::enqueueExits(mlir::Operation *op) {
  ...
  if (auto loopOp = mlir::dyn_cast<cir::LoopOpInterface>(op)) {
    loopOp->walk([&](mlir::Operation *op) {
      ...
      if (auto breakOp = mlir::dyn_cast<cir::BreakOp>(op)) {
        auto breakTarget = getBreakTarget(breakOp);
  ...
  if (auto switchOp = mlir::dyn_cast<cir::SwitchOp>(op)) {
    switchOp->walk([&](cir::BreakOp breakOp) {
      ...
      mlir::Operation *breakTarget = getBreakTarget(breakOp);
...
void LiveObjectDiagnosticPass::runOnOperation() {
  ...
  getOperation()->walk([&](mlir::Operation *op) { dump(liveObjects, op); });
}

std::unique_ptr<mlir::Pass> mlir::createLiveObjectDiagnosticPass() {
  return std::make_unique<LiveObjectDiagnosticPass>();
}
