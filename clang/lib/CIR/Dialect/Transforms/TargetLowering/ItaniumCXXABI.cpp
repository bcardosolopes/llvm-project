...
static mlir::Value lowerMethodCast(mlir::Operation *op, mlir::Value loweredSrc,
  ...
  // Reconstruct the record with the adjusted adj field.
  // Extract ptr (index 0), then store both fields into an alloca and reload.
  auto ptrField = cir::ExtractMemberOp::create(builder, op->getLoc(),
                                               ptrdiffCIRTy, loweredSrc, 0);
  auto recordTy = mlir::cast<cir::RecordType>(loweredSrc.getType());
  auto recordPtrTy = cir::PointerType::get(recordTy);
  auto alloca = cir::AllocaOp::create(
      builder, op->getLoc(), recordPtrTy, recordTy, /*name=*/"",
      mlir::IntegerAttr::get(mlir::IntegerType::get(builder.getContext(), 64),
                             8));

  auto ptrMemberTy = cir::PointerType::get(ptrdiffCIRTy);
  auto ptrMember = cir::GetMemberOp::create(builder, op->getLoc(), ptrMemberTy,
                                            alloca, /*name=*/"", /*index=*/0);
  cir::StoreOp::create(builder, op->getLoc(), ptrField, ptrMember,
  ...
  auto adjMember = cir::GetMemberOp::create(builder, op->getLoc(), ptrMemberTy,
                                            alloca, /*name=*/"", /*index=*/1);
  cir::StoreOp::create(builder, op->getLoc(), adjustedAdjField, adjMember,
                       /*is_volatile=*/false, /*is_nontemporal=*/false,
                       /*alignment=*/mlir::IntegerAttr{},
                       /*sync_scope=*/cir::SyncScopeKindAttr{},
                       /*mem_order=*/cir::MemOrderAttr{},
  ...
  return cir::LoadOp::create(builder, op->getLoc(), alloca,
                             /*isDeref=*/false, /*isVolatile=*/false,
                             /*is_nontemporal=*/false,
                             /*alignment=*/mlir::IntegerAttr{},
                             /*sync_scope=*/cir::SyncScopeKindAttr{},
