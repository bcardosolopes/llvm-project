...
namespace mlir {
#define GEN_PASS_DEF_POINTSTODIAGNOSTIC
#include "clang/CIR/Dialect/Passes.h.inc"
} // namespace mlir

struct PointsToDiagnosticPass
    : public mlir::impl::PointsToDiagnosticBase<PointsToDiagnosticPass> {
...
bool PointsToAnalysis::handleSpecialCall(cir::CallOp call) {
  ...
  auto cxxSpecialMember = calleeFunc.getCxxSpecialMemberAttr();
  if (!cxxSpecialMember)
    return false;
...
void PointsToAnalysis::handleCall(cir::CallOp call) {
  ...
  for (mlir::Value arg : argOperands) {
  ...
  if (mayWrite) {
    ...
    llvm::SetVector<mlir::Value> closure(call.getArgOperands().begin(),
...
std::unique_ptr<mlir::Pass> mlir::createPointsToDiagnosticPass() {
  return std::make_unique<PointsToDiagnosticPass>();
}
