...
//===----------------------------------------------------------------------===//
//
// This provides a concrete class for CUDA CIR generation targeting
// NVIDIA CUDA and HIP runtimes.
//
//===----------------------------------------------------------------------===//
...
void CIRGenNVCUDARuntime::emitDeviceStubBodyNew(CIRGenFunction &cgf,
  ...
  for (auto [i, arg] : llvm::enumerate(args)) {
    ...
    mlir::Value argAddrVoidPtr = builder.createBitcast(argAddr, cgm.voidPtrTy);
    builder.CIRBaseBuilderTy::createStore(loc, argAddrVoidPtr, storePos);
  }
  ...
  // int __[cuda|hip]PopCallConfiguration(dim3 *gridDim, dim3 *blockDim,
  // size_t
...
mlir::Operation *CIRGenNVCUDARuntime::getKernelHandle(cir::FuncOp fn,
  ...
  if (auto *existing = cgm.getGlobalValue(globalName))
    globalOp = mlir::cast<cir::GlobalOp>(existing);
  else {
    globalOp = CIRGenModule::createGlobalOp(
        cgm, fn->getLoc(), globalName,
        cir::PointerType::get(fn.getFunctionType()), /*isConstant=*/true);
  }
...
void CIRGenNVCUDARuntime::finalizeModule() {
  ...
  // eliminated by optimization. NYI: addCompilerUsedGlobal not yet available.
  assert(!cir::MissingFeatures::cudaDeviceVarODRUsed());
}
