...
public:
  ...
  matchAndRewrite(cir::GlobalOp op, OpAdaptor adaptor,
    ...
    cir::ConstVectorAttr savedVecAttr;
    ...
    if (init.has_value()) {
      ...
      } else if (auto vecAttr =
                     mlir::dyn_cast<cir::ConstVectorAttr>(init.value())) {
        // memref.global does not support vector element types in its initial
        // value (the verifier requires TensorType, but DenseElementsAttr
        // cannot represent tensor-of-vector). Leave the global uninitialized
        // here; CIRCopyOpLowering will inline the constant at the use site.
        // Save the original CIR attribute for CIRCopyOpLowering to find.
        savedVecAttr = vecAttr;
        initialValue = mlir::Attribute();
      } else
        llvm_unreachable(
    ...
    auto newGlobal = rewriter.replaceOpWithNewOp<mlir::memref::GlobalOp>(
    ...
    // Save the vector constant attribute on the memref.global so that
    // CIRCopyOpLowering can inline it at the use site.
    if (savedVecAttr)
      newGlobal->setAttr("cir.const_vector_init", savedVecAttr);
...
public:
  ...
  matchAndRewrite(cir::CopyOp op, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const override {
    // Check if the source is a get_global referencing a constant vector global.
    // memref.global cannot represent vector initial values, so we inline the
    // constant as arith.constant + memref.store instead of memref.copy.
    if (auto getGlobal = op.getSrc().getDefiningOp<cir::GetGlobalOp>()) {
      auto moduleOp = op->getParentOfType<mlir::ModuleOp>();
      if (moduleOp) {
        auto sym = moduleOp.lookupSymbol(getGlobal.getName());
        if (auto memrefGlobal =
                mlir::dyn_cast_or_null<mlir::memref::GlobalOp>(sym)) {
          // Check if the memref.global has a saved vector constant attribute.
          if (auto vecAttr = memrefGlobal->getAttrOfType<cir::ConstVectorAttr>(
                  "cir.const_vector_init")) {
            auto convertedType =
                getTypeConverter()->convertType(vecAttr.getType());
            auto vecTy = mlir::cast<mlir::VectorType>(convertedType);
            SmallVector<mlir::Attribute> mlirValues;
            for (auto elementAttr : vecAttr.getElts()) {
              if (auto intAttr = mlir::dyn_cast<cir::IntAttr>(elementAttr))
                mlirValues.push_back(mlir::IntegerAttr::get(
                    vecTy.getElementType(), intAttr.getValue()));
              else if (auto fpAttr = mlir::dyn_cast<cir::FPAttr>(elementAttr))
                mlirValues.push_back(mlir::FloatAttr::get(
                    vecTy.getElementType(), fpAttr.getValue()));
            }
            auto denseAttr = mlir::DenseElementsAttr::get(vecTy, mlirValues);
            auto constOp = mlir::arith::ConstantOp::create(
                rewriter, op.getLoc(), vecTy, denseAttr);
            auto zeroIndex =
                mlir::arith::ConstantIndexOp::create(rewriter, op.getLoc(), 0);
            // Look through memref.cast to find the underlying alloca with
            // static shape for the store destination.
            mlir::Value dst = adaptor.getDst();
            if (auto castOp = dst.getDefiningOp<mlir::memref::CastOp>())
              dst = castOp.getSource();
            mlir::memref::StoreOp::create(rewriter, op.getLoc(),
                                          constOp.getResult(), dst,
...
convertCIRLangAddrSpaceToGPU(cir::LangAddressSpaceAttr addrSpace) {
  ...
  llvm_unreachable("Unknown LangAddressSpace");
}
